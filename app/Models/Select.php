        ...
        switch ($vcSelect) {
            ...
            case 'events_activities':
                $oSelect = EventsActivity::select('id as value', 'name->'.app()->getLocale().' as option')
                    ->where('events_activities.events_id', $parameter1)
            ...
            case 'events_payments':
                $oSelect = EventsPayment::select('id as value', 'description->'.app()->getLocale().' as option')
                    ->where('events_payments.events_id', $parameter1)
            ...
            case 'provinces':
                ...
                break;
            case 'roles':
                $oSelect = Role::select('id as value', 'description->'.app()->getLocale().' as option')
                    ->where('townhalls_id', session('townhall_id'))
                    ->orderBy('description->'.app()->getLocale(), 'asc')
                    ->get()->toArray();
                break;
            case 'schedules':
                $oSelect = Schedule::select('schedules.id as value', 'schedules.description->'.app()->getLocale().' as option')
                    ...
                    ->where(function ($query2) use ($parameter1) {
                        return $query2->where('us.users_id', $parameter1)
                            ->orWhere('ug.users_id', $parameter1);
                    })
            ...
            case 'services_events_assigned_users':
                $oSelect = User::select('users.id as value', 'users.name as option')
                    ->join('services_events_notes as sen', function ($join) {
                        $join->on('sen.assigned_user_id', 'users.id');
                        // ->where('sm.model', $parameter1)
                    })
                    ->join('services_events as se', function ($join) {
            ...
            case 'shows_events':
                $oSelect = ShowsEvent::select('id as value', 'description->'.app()->getLocale().' as option')
                    ->where('shows_id', $parameter1)
                    ->when(! empty($parameter2), function ($query) {
                        $query->where('tickets_to_date', '>', now());
                    })
                    ->when(! empty($parameter3), function ($query) {
                        $query->where('to_date', '>=', today());
                    })
                    ->when(! empty($parameter4), function ($query) {
            ...
            case 'sports_events_activities':
                $oSelect = SportsEventsActivity::select('id as value', 'name->'.app()->getLocale().' as option')
                    ->where('sports_events_activities.events_id', $parameter1)
            ...
            case 'sports_events_payments':
                $oSelect = SportsEventsPayment::select('id as value', 'description->'.app()->getLocale().' as option')
                    ->where('sports_events_payments.events_id', $parameter1)
            ...
            case 'sports_installations_resources_groups':
                $oSelect = SportsInstallationsResourcesGroup::select('id as value', 'name->'.app()->getLocale().' as option')
                    // ->where('townhalls_id', session('townhall_id'))
                    ->where('sports_installations_resources_groups.installations_id', $parameter1)
                    ->orderBy('name->'.app()->getLocale(), 'asc')
                    ->get()->toArray();
                break;
            ...
            case 'templates':
                $oSelect = Template::select('id as value', 'description as option')
                    ->when(! empty($parameter1), function ($query) use ($parameter1) {
                        $query->where('tsections_id', $parameter1);
                    })
                    ->when(! empty($parameter2), function ($query) use ($parameter2) {
                        $query->where('tobjects_id', $parameter2);
                    })
                    ->when(! empty($parameter3), function ($query) use ($parameter3) {
                        $query->where('ttypes_id', $parameter3);
                    })
                    ->orderBy('description', 'asc')
                    ->get()->toArray();
                break;
