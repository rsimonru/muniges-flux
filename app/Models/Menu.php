...
class Menu extends Model
{
    ...
    public static function emtGet(
        ...
        ->when(isset($filters['iLevel']) && $filters['iLevel']>0, function($query) use ($filters) {
            return $query->where('menus.level', '<=', $filters['iLevel']);
        })
        ...
        ->when(isset($filters['iUsers_id']) && $filters['iUsers_id']>0, function($query) use ($filters) {
            ...
            ->when(isset($filters['iLevel']) && $filters['iLevel']>0, function($coll) use ($filters) {
                return $coll->where('level', '<=', $filters['iLevel']);
            })
